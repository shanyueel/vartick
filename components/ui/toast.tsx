...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  const iconClass = "rounded-full p-0.5"

  if (type === "success") {
    icon = <CheckIcon className={cn(iconClass, "text-success bg-success/20")} aria-hidden="true" />
  }
  ...
  if (type === "info") {
    icon = (
      <span
        className={cn(
          iconClass,
          "block align-middle size-5 text-center text-xs text-info bg-info/20 "
        )}
        aria-hidden="true"
      >
        i
  ...
  if (type === "warning") {
    icon = (
      <span
        className={cn(
          iconClass,
          "block align-middle size-5 text-center text-xs text-warning bg-warning/20 "
        )}
        aria-hidden="true"
      >
        !
  ...
  if (type === "error") {
    icon = <XIcon className={cn(iconClass, "text-error bg-error/20")} aria-hidden="true" />
  }
  ...
  return toasts.map((toastItem) => (
    <Toast key={toastItem.id} toast={toastItem}>
      <ToastContent>
        ...
        <ToastAction />
        {toastItem.data?.showCloseButton !== false && <ToastClose />}
      </ToastContent>
    </Toast>
  ))
}
