...
| *Weekend reserve* | PWA manifest + service worker + Lighthouse pass; Playwright flow #1. |
...
| *Weekend reserve* | Playwright flow #2. |
...
| **Thu** | Adjustment actions — *shift* first and always; *reschedule* and *drop* only if Wednesday finished clean. |
| **Fri** | Nudge notifications; plan-vs-actual view; design pass. **Tag `v0.3`, deploy.** |
| *Weekend reserve* | Playwright flow #3. |
...
| *Weekend reserve* | Third and fourth LLM adapters; JSON import. |
...
| R7 | **Design polish deferred to the end** and then cut. | Medium | High | Design passes are scheduled *within* each week (Friday, before the tag), not batched at the end. S5 is a success criterion, not a nice-to-have. |
